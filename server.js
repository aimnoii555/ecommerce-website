...
let serviceAccount = require("./ecom-website-fa2a3-firebase-adminsdk-xfeny-6fee4695c8.json");
...
const bucketName = 'ecom-website-turtorial-2'
// const accessKeyId = process.env.AWS_ACCESS_KEY
// const secretAcessKey = process.AWS_SECRET_KEY
...
aws.config.update({
    ...
    apiVersion: 'latest',
    credentials: {
      accessKeyId: 'AKIARQKRD2SBJBNYGAN4',
      secretAccessKey: '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'
    }
  })
...
app.post('/seller',(req,res)=> {
    ...
    if (!name.length || !address.length || !about.length || number.length < 10 || !Number(number)) {
        return res.json(
            {
                'alert': 'some information(s) is/are invaid'
            }
        )
    } else if (!tac || !legit) {
        return res.json(
            {
                'alert': 'you must agree to our terms and condtions'
            }
        )
    } else {
     ...
     if (!draft) {
        if (!name.length) {
            return res.json({'alert':'enter product name'})
             ...
             return res.json({'alert':'you should have at least 20 item in stock'})
         } else if (!tags.length) {
...
//     let docName = email + Math.floor(Math.random() * 123456789987654321)
...
//                 res.json(
//                     {
//                         'alert': 'opps! its seems like some err occured. Try Again'
//                     }
//                 )
//             } else {
//                 res.json(
